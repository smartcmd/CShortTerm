...
本项目选用了[Xmake](https://xmake.io/#/)作为构建系统。`Xmake`是一款现代化，高效率，基于`Lua`的轻量级跨平台编译工具。与`Makefile`或 `CMake`相比，`Xmake`的配置语法更加简洁直观，且提供了C/C++所缺失的包管理功能。因此，它对新手很友好，同时还能保持构建系统所需的灵活性。
